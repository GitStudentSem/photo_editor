...
const Settings = observer(() => {
	...
	const [isFlop, setIsFlop] = useState(false);
	...
	async function sendPhoto() {
		...
		for (let i = 0; i < PhotoStore.photo.length; i++) {
			...
			data.append('flip', String(isFlip))
			data.append('flop', String(isFlop))
			data.append("image", PhotoStore.photo[i] as Blob);
			const res: any = await fetch("http://localhost:3333/flip", {
				method: "POST",
				body: data,
			});
	...
	return (
		<div className={styles.settings}>
			...
			<label>
				...
				<Checkbox onChange={prev => setIsFlip(!prev)} disabled={!PhotoStore.photo} />
			</label>
			<label>
				Отразить по вертикали
				<Checkbox onChange={prev => setIsFlop(!prev)} disabled={!PhotoStore.photo} />
			</label>
			<Button type="submit"
				onClick={sendPhoto}
